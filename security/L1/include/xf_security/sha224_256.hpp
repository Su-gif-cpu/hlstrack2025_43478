...
// 优化的内联函数 - 展开所有操作，减少函数调用开销
static inline uint32_t ROTR(int n, uint32_t x) {
#pragma HLS inline
    return (x >> n) | (x << (32 - n));
}

static inline uint32_t SHR(int n, uint32_t x) {
#pragma HLS inline
    return x >> n;
}
...
static inline void preProcessing(hls::stream<ap_uint<32> >& msg_strm,
...
/// @param nblk_strm the number of hash block for this message.
/// @param end_nblk_strm end flag for number of hash block.
static inline void preProcessing(hls::stream<ap_uint<64> >& msg_strm,
                          hls::stream<ap_uint<64> >& len_strm,
                          hls::stream<bool>& end_len_strm,
                          hls::stream<SHA256Block>& blk_strm,
                          hls::stream<uint64_t>& nblk_strm,
...
static inline void dup_strm(hls::stream<uint64_t>& in_strm,
...
static inline void generateMsgSchedule(hls::stream<SHA256Block>& blk_strm,
                                hls::stream<uint64_t>& nblk_strm,
                                hls::stream<bool>& end_nblk_strm,
...
#pragma HLS pipeline II = 2 rewind
                // 优化：展开2次提高吞吐量
                // Round 1
                // uint32_t ssig1_0 = SSIG1(W[14]);
                // uint32_t ssig0_0 = SSIG0(W[1]);
                // uint32_t Wt0 = ssig1_0 + W[9] + ssig0_0 + W[0];
                uint32_t ssig1_0 = SSIG1(W[14]);
                uint32_t ssig0_0 = SSIG0(W[1]);
                uint32_t w_sum1_0 = ssig1_0 + W[9];      // 并行加法1
                uint32_t w_sum2_0 = ssig0_0 + W[0];      // 并行加法2
                uint32_t Wt0 = w_sum1_0 + w_sum2_0;    // 最终加法
...
#pragma HLS unroll
                ...
                // Round 2
                // uint32_t ssig1_1 = SSIG1(W[14]);
                // uint32_t ssig0_1 = SSIG0(W[1]);
                // uint32_t Wt1 = ssig1_1 + W[9] + ssig0_1 + W[0];
                uint32_t ssig1_1 = SSIG1(W[14]);
                uint32_t ssig0_1 = SSIG0(W[1]);
                uint32_t w_sum1_1 = ssig1_1 + W[9];      // 并行加法1
                uint32_t w_sum2_1 = ssig0_1 + W[0];      // 并行加法2
                uint32_t Wt1 = w_sum1_1 + w_sum2_1;    // 最终加法
...
static inline void sha256_iter(uint32_t& a,
                        uint32_t& b,
                        uint32_t& c,
                        uint32_t& d,
                        uint32_t& e,
...
#pragma HLS inline
    ...
    // 并行计算两条路径
    uint32_t T1_part1 = h + Kt + Wt;
    uint32_t T1_part2 = BSIG1(e) + CH(e, f, g);
    uint32_t T1 = T1_part1 + T1_part2;
...
#pragma HLS loop_tripcount min = 1 max = 1
            ...
            // loading working variables - 优化：展开加载
            a = H[0]; b = H[1]; c = H[2]; d = H[3];
            e = H[4]; f = H[5]; g = H[6]; h = H[7];

            // 优化：展开2次，减少迭代次数，降低总延迟
        LOOP_SHA256_UPDATE_64_ROUNDS:
            for (short t = 0; t < 64; t += 2) {
#pragma HLS pipeline II = 2 rewind
                // Round t - 拆分计算，减少加法器级联深度
                uint32_t Wt0 = w_strm.read();
                uint32_t Kt0 = K[t];
                
                uint32_t ch0 = CH(e, f, g);
                uint32_t bsig1_0 = BSIG1(e);
                uint32_t maj0 = MAJ(a, b, c);
                uint32_t bsig0_0 = BSIG0(a);
                
                // 分3级加法，平衡关键路径
                uint32_t T1_temp0a = h + ch0;
                uint32_t T1_temp0b = bsig1_0 + Kt0;
                uint32_t T1_0 = T1_temp0a + T1_temp0b + Wt0;
                uint32_t T2_0 = bsig0_0 + maj0;

                uint32_t a_new = T1_0 + T2_0;
                uint32_t e_new = d + T1_0;
                ...
                // Round t+1 - 同样拆分计算
                uint32_t Wt1 = w_strm.read();
                uint32_t Kt1 = K[t + 1];
                
                uint32_t ch1 = CH(e, f, g);
                uint32_t bsig1_1 = BSIG1(e);
                uint32_t maj1 = MAJ(a, b, c);
                uint32_t bsig0_1 = BSIG0(a);
                
                uint32_t T1_temp1a = h + ch1;
                uint32_t T1_temp1b = bsig1_1 + Kt1;
                uint32_t T1_1 = T1_temp1a + T1_temp1b + Wt1;
                uint32_t T2_1 = bsig0_1 + maj1;
                
                // Final update
                h = g; g = f; f = e; e = d + T1_1;
                d = c; c = b; b = a; a = T1_1 + T2_1;
            } // 64 round loop (unrolled 2x)

            // store working variables to internal states - 优化：展开存储
...
#pragma HLS RESOURCE variable = end_nblk_strm1 core = FIFO_SRL
    hls::stream<bool> end_nblk_strm2("end_nblk_strm2");
#pragma HLS STREAM variable = end_nblk_strm2 depth = 8
#pragma HLS RESOURCE variable = end_nblk_strm2 core = FIFO_SRL
